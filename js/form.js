...
const setCoordinates = () => {
  let leftPoint = Math.floor(window.pins.Locations.X_MIN - window.pins.Properties.MAIN_WIDTH / 2);
  let rightPoint = Math.floor(window.pins.Locations.X_MAX - window.pins.Properties.MAIN_WIDTH / 2);
  let bottomPoint = window.pins.Locations.Y_MAX - window.pins.Properties.MAIN_HEIGHT;
  let height;
  let x;
  let y;
